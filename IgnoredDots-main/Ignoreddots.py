...
def calculate_distances(locations):
    ...
    for i, (lat, lon) in enumerate(locations):
        if prev_loc:
            ...
            if segment_distance > 10:  # Threshold for large jumps
...
for item in data:
    ...
    ride_data = {
        ...
        'Monthly Remaining Tokens': item.get('monthlyRemainingTokens', 150),
...
with pd.ExcelWriter('test.rides19.6.xlsx', engine='openpyxl') as writer:
    df.to_excel(writer, sheet_name='Ride Summary', index=False)

print("Data written to Excel file 'test.rides19.6.xlsx' successfully.")